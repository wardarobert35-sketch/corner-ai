        ...
        if not mecze:
            ...
            for mecz in mecze:
                ...
                with st.spinner(f"Analizuję {mecz['homeTeam']['name']}..."):
                    ...
                    szansa = oblicz_prawdopodobienstwo(suma)
                    ...
                    if szansa >= minimalna_szansa:
                        with st.expander(f"✅ {mecz['homeTeam']['name']} vs {mecz['awayTeam']['name']} - {szansa:.1f}%", expanded=True):
                            ...
                            col3.metric("Szansa", f"{szansa:.1f}%")
...
with tab2:
    ...
    kurs_buka = st.number_input("Kurs u bukmachera (np. 1.85)", value=1.80)
    ...
    if st.button("ANALIZUJ MECZ"):
        ...
        st.info(f"Szansa: {szansa:.1f}% | Kurs sprawiedliwy: {100/szansa:.2f}")
